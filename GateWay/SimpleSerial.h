...
public:
     ...
     * serial device
     */
    SimpleSerial(std::string port, unsigned int baud_rate , int parity ,int character,int stop)
        : io(), serial(io, port)
    {
        serial.set_option(boost::asio::serial_port_base::baud_rate(baud_rate));
        serial.set_option(boost::asio::serial_port_base::character_size(character));
        switch (parity){
            case 0:
